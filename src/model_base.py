...
class ModelBase(State):
    ...
    def calc_inputs(self, t: np.float64, p: ParameterSet):
        ...
        logging.debug(
            f'[{t:.4f}] input:   {arr2str([getattr(self,x)._input for x in self.Nodes])}')
        return
    ...
    def calc_phis(self, t: np.float64, p: ParameterSet):
        ...
        logging.debug(
            f'[{t:.4f}] phi:     {arr2str([getattr(self,x)._phi for x in self.Nodes])}')
        return
    ...
    def calc_ds(self, delta: 'ModelBase', t: np.float64, params: ParameterSet):
        ...
        logging.debug(
            f'[{t:.4f}] ds:      {arr2str([getattr(delta,x).s for x in self.Nodes])}')
        logging.debug(
            f'[{t:.4f}] ds_ampa: {arr2str([getattr(delta,x).s_ampa for x in self.Nodes])}')
        return
    ...
    def calc_dr(self, delta: 'ModelBase', t: np.float64, params: ParameterSet):
        ...
        logging.debug(
            f'[{t:.4f}] dr:      {arr2str([getattr(delta,x).r for x in self.Nodes])}')
        return
    ...
    def calc_dy(self, delta: 'ModelBase', t: np.float64, params: ParameterSet):
        ...
        logging.debug(
            f'[{t:3.4f}] dy:      {arr2str([getattr(delta,x).y for x in self.Nodes])}')
        return

    def calcDelta(self, t: np.float64, params: ParameterSet):
        logging.debug(f'[{t:.4f}] ')
